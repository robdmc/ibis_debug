...
build: ## Build docker images locally and don't send to dockerhub
	...
	docker-compose down
	docker rmi -f tester/ibis_debug
	docker build -t  tester/ibis_debug .
...
reset: ## Blow away all docker resourses associated with this project
	docker-compose down
	-docker volume rm ibis_debug_db-data
	-docker volume rm ibis_debug_env
	-docker images | grep 'ibis_debug' | awk '{print $3}'  | xargs docker rmi
	-docker builder prune -f
